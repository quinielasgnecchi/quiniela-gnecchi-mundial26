...
export default function AuthPage() {
  ...
  async function handleRegister() {
    ...
    // Extrae de forma limpia el usuario del correo (jpgnecchi)
      ...
      <div className="w-full max-w-sm" style={{ background: '#141414', border: '1px solid #1f1f1f', borderRadius: '20px', padding: '24px' }}>
        <div className="flex rounded-xl p-1 mb-6" style={{ background: '#0a0a0a' }}>
          {(['login', 'register'] as Mode[]).map(m => (
            <button key={m} onClick={() => { setMode(m); setError(''); setSuccess('') }} className="flex-1 py-2.5 rounded-lg text-sm font-medium transition-all" style={mode === m ? { background: '#244ffe', color: 'white' } : { color: '#666' }}>
        ...
        <div className="flex flex-col gap-4">
          {mode === 'register' && (
            <div className="flex flex-col items-center gap-2">
              <label className="cursor-pointer">
                <div className="w-20 h-20 rounded-full flex items-center justify-center overflow-hidden" style={{ background: '#1a1a1a', border: '2px dashed #2a2a2a' }}>
          ...
          <button onClick={mode === 'login' ? handleLogin : handleRegister} disabled={loading} className="w-full py-3.5 rounded-xl font-semibold text-white" style={{ background: '#244ffe', opacity: loading ? 0.6 : 1 }}>
